...
| 2026-09-09 | [`2026-09-09-tablet-my-work-left.md`](2026-09-09-tablet-my-work-left.md) — the tablet's numbers become per-person: the card and the header count down what the signed-in person has left (`total` less that stage's counter, summed over each stage they hold, so every tap moves it by one), so two people at one tablet see different numbers | in progress |
| 2026-09-10 | [`2026-09-10-glass-colours-two-way.md`](2026-09-10-glass-colours-two-way.md) — **the floor's work reaches the `Production` sheet**: the office dashboard paints DG, TG, TUFF and NOT TUFF yellow and gold from the floor's counters (fills only, fully reversible, last writer wins); a TUFF counter on the tablet with its own total; and `OfficeDone`, which makes a job read-only on the tablet. Read its Amendments section: it records one gap in the brief (`TuffTotal`) and every decision taken | built 2026-09-10, tests green — not demoed, not committed |
