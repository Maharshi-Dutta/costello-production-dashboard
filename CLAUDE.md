...
2. **No other sheet on the workbook is touched** except the dashboard's own
   ...
   for the dashboard — it is never created or written by code, only read.
   `Dashboard Progress` was one of them until 2026-09-11. It is now
   **frozen**: the counts moved to the `Dashboard progress` SharePoint list
   and nothing writes the sheet again. It is parsed out of the downloaded
   workbook for **one** reader — the one-time import of the sheet's colours —
   and not parsed at all once that import has drained, which is every load
   but the first few, ever. `saveProgress`/`saveProgressMany` stay in
   `graph.js`, unused.
3. **SharePoint lists** hold state the workbook cannot hold safely: in the
   workbook's own site, `Dashboard phases` (hand-set phase per job),
   ...
   **`Dashboard progress` — the single record of checkpoint status, one row
   ...
   `Dashboard progress` is the one list whose content is deliberately
