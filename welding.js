/* The Welding station page - the PVC welders' own screen.
   ...
   export, and no link back to the master dashboard. The station account cannot
   reach the workbook and this page would not know what to do with it if it
   could.
   ...
   The two station pages share station-core.js (the list mechanics, the people,
   the log lines, the note channel and the delta merge), welding-core.js's
   `WELD` definition for this station's own columns and rules, and
   station-ui.js for the theme, the gate and the picker. What is here is this
   page: its queue, its board and its drawing.                               */

const $ = s => document.querySelector(s);
const esc = STU.stuEsc;
const W = WELDC;

/* This page's own storage keys. Deliberately NOT the glass page's: the same
   tablet can have both pages open, and a welder's queue must never be read as
   a glass tap. The theme is the one thing they share - it belongs to the
   device (STU_THEME_KEY). */
...
let ITEMS = [];                 // the Welding station list, as last read
...
/* One stage here, so anybody signed in holds it. canStage is still asked -
   a Stages column typed by hand can say anything, and a person holding nothing
   may move nothing, on this station exactly as on the glass one. */
...
/* ---- the queues -------------------------------------------------------------
   ...
   back through welding-core's own whitelist, because anyone holding the tablet
...
function cleanLogQ(raw) {
  ...
  Object.keys(raw || {}).forEach(k => {
    ...
    out[String(k)] = { key: String(k), err: 0, fields: ST.logFields({
      job: e.fields.Title, station: W.WELD_NAME, type: e.fields.GlassType,
...
function queueTap(rec, part, value) {
  ...
  const had = QUEUE[k];
  /* `from` belongs to the moment this row and part first went into the queue:
     ...
     means anything in the list it came from. The Title is stamped on for the
     same reason and is what finds the row again if the lists ever move. */
...
function queuedFor(id) {
  ...
  Object.keys(QUEUE).forEach(k => {
    ...
    out[W.WELD_DONE_FIELD[e.part]] = e.value;
  });
...
/** The same two questions asked of a whole card, because the card is what says
    "saving..." and the group rows under it are what moved. */
...
function listValue(id, part) {
  ...
  if (!it) return 0;
  const v = Number((it.fields || {})[W.WELD_DONE_FIELD[part]]);
  return isFinite(v) ? Math.round(v) : 0;
}

/** Re-base or drop every queued tap against the list as it now stands. The
    three answers, and why each is the right one, are in welding-core's
    weldRebase - this is the part that has a queue and a screen to tell. */
function rebaseQueue() {
  ...
  Object.keys(QUEUE).forEach(k => {
    ...
    if (!it) return;
    const r = W.weldRebase(e, it.fields || {});
    if (r.action === "drop") {
      ...
      LOST[job] = note;
      console.warn("[welding] dropping a queued tap for " + e.job + " " + e.group + ": " +
        W.weldPartLabel(e.part) + " " + e.value + " was tapped at " + e.at +
        ", and the row was moved after that (" + ((it.fields || {}).DoneAt || "") +
        ") - tap it again if it is still right");
      delete QUEUE[k];
      moved = true;
      return;
    }
  ...
  try {
    for (;;) {
      ...
      /* WHAT WENT, CAPTURED BEFORE THE AWAIT AND NOT READ BACK OFF THE ENTRY.
         ...
         air would be deleted as though it had been sent. These four locals are
         what this pass actually sent; everything below decides against them.
         (station.js has the same shape and is deliberately left alone here -
         it is inherited, not introduced, and touching it would mean touching
         the frozen glass suite. Recorded in docs/HISTORY.md B19.) */
      const want = e.value, when = e.at, whose = e.who, wantFrom = e.from;
      const body = W.weldFloorOnly(W.weldTapFields(e.part, want, whose, when));
      ...
      try {
        await CW.listPatch(W.WELD_LIST, id, body, listOpts());
        wrote = true;
        /* the counter is in the list now, so the log line describing it may be
           ...
           be a lie in a list nothing ever deletes from. It describes the write
           that went, so it is built from the captured values too. */
        ...
        const it = ITEMS.find(x => String(x.id) === String(id));
        if (it) (it.fields = it.fields || {})[W.WELD_DONE_FIELD[e.part]] = want;
      } catch (err) {
        ...
        if (CW.isMissing && CW.isMissing(err) && CW.forgetStationSite)
          CW.forgetStationSite(false, W.WELD.site);
        console.warn("[welding] counter write failed for item " + id + ":", (err && err.message) || err);
      }
      saveQueue();
    }
    await flushLog();
...
function queueLog(e) {
  ...
  LOGQ[key] = { key: key, err: 0, fields: ST.logFields(W.weldLogEntry({
...
async function flushLog() {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    try {
      ...
      e.err = 1;
      console.warn("[welding] log line not written yet:", (err && err.message) || err);
    }
    saveQueue();
  }
}
...
/* ---- the lists ---- */
let SITEID = null;
const listOpts = () => ({ siteId: SITEID, fields: W.WELD_FIELDS });
...
/* ---- a word for the office -------------------------------------------------
   ...
   the two list calls - is in station-core.js; `station` is the only
   station-specific input it takes. That is the whole of what a second station
   had to do for this feature, and it is why the feature was built that way. */
...
function trouble(e) {
  ...
  console.warn("[welding] could not read SharePoint:", m);
  if (CW.isMissing && CW.isMissing(e)) {
    SITEID = null;
    if (CW.forgetStationSite) CW.forgetStationSite(false, W.WELD.site);
  }
...
/** The site this station's lists are in: `Floor stations` and nothing else
    (W.WELD.site === "floor"). A missing one is not an error and not a fallback
    - it is the quiet explained state, the same one a missing list gets. */
async function resolveSite() {
  if (!SITEID) SITEID = await CW.stationSite(W.WELD.site);
  return SITEID;
}
...
async function readPeople() {
  try {
    ...
    /* this station's people, and this station's one stage */
    PEOPLE = ST.stationPeople(items, W.WELD_NAME, W.WELD.stages);
    PEOPLE_READ = true;
...
async function readList() {
  try {
    ...
    let items = null;
    if (deltaOff()) {
      items = await CW.listItems(W.WELD_LIST, listOpts());
      if (items == null) { PROBLEM = "list"; SOFT = ""; READY = false; render(); return false; }
      TOKEN = null;
    } else {
      try {
        const d = await CW.listDelta(W.WELD_LIST, listOpts());
        ...
        if (!(CW.isDeltaResync && CW.isDeltaResync(e))) {
          DELTA_OFF = Date.now();
          console.warn("[welding] the board list refused a delta; polling the plain way for five minutes");
        }
        items = await CW.listItems(W.WELD_LIST, listOpts());
        if (items == null) { PROBLEM = "list"; SOFT = ""; READY = false; render(); return false; }
        TOKEN = null;
...
let SITE_GEN = 0;
function siteMoved() {
  const m = CW.stationSiteMoves ? CW.stationSiteMoves(W.WELD.site) : 0;
...
async function pollList() {
  ...
  try {
    const d = await CW.listDelta(W.WELD_LIST, { siteId: SITEID, fields: W.WELD_FIELDS, token: TOKEN });
...
function tap(id, part, delta) {
  /* belt and braces: the stepper is drawn disabled for somebody who holds no
     ...
     this is the rule */
  if (!PERSON || !mayWeld()) return;
  const rec = recordById(id);
  if (!rec) return;
  /* somebody is working the screen, whether or not the number could move */
  touch();
  const value = W.weldApplyTap(rec, part, delta);
  if (value == null || value === rec[part]) return;      // already at the clamp
...
/* ---- drawing ---- */
/** A progress bar as ten blocks, because a workshop screen is read at arm's
    length and a 3px rule is not. Width, not characters, so a 49-frame job and
    a 1-frame job draw the same shape. */
...
/** One part of one group: the name, the count, the bar and the three buttons
    that move it - on the card itself, where a hand can reach them. Nothing to
    open, nothing to scroll. Every target is at least 56 px, because it is
    tapped with a work glove on. */
function stepHtml(rec, line) {
  const mine = mayWeld();
  const off = mine ? "" : ' disabled aria-disabled="true"';
  const b = (t, act, cls, dead) => '<button class="' + cls + '" data-id="' + esc(rec.id) +
...
/** One product group inside a card: its name, its colour, and its drawn lines.
    A line with nothing to weld is not drawn (welding-core does that); a group
    with nothing to weld is never fed at all. */
function groupHtml(rec) {
  return '<div class="wgrp c-' + (rec.colour || "none") + '" data-grp="' + esc(rec.id) + '">' +
...
function cardInner(c) {
  const owed = owedForCard(c), bad = badForCard(c);
  const lost = lostFor(c.job);
  const qty = W.weldQtyWords(c);
  return '<div class="chead">' +
    ...
    (lost ? '<div class="unsaved">' + esc(lost.parts.map(p =>
        p.group + " " + W.weldPartLabel(p.part) + " " + p.value).join(", ")) +
...
function pickerHtml() {
  return STU.stuPickerHtml({
    ...
    labelOf: () => "welding",
    empty: "Nobody is set up for the Welding station yet. Ask the office to add " +
...
function words() {
  return PROBLEM === "reauth"
    ...
    : PROBLEM === "list"
    ? "The “Welding station” list is not in the floor’s site yet. Ask the office to add it — " +
...
function wireAgain() {
  ...
  if (a) a.onclick = () => {
    ...
    if (CW.forgetStationSite) CW.forgetStationSite(true, W.WELD.site);   // a person asked: look now
    render(); readPeople(); readList();
  };
  const r = $("#reauth");
  if (r) r.onclick = async () => {
    try { await CW.signIn(CW.LIST_SCOPES); PROBLEM = ""; SITEID = null; TOKEN = null; await start(); }
    catch (e) { console.warn("[welding] sign-in again failed:", (e && e.message) || e); }
  };
}
